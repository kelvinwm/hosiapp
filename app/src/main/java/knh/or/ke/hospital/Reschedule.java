...
import knh.or.ke.hospital.Adapters.RescheduleAdapter;
...
public class Reschedule extends AppCompatActivity {
    ...
    String clinname,reskey,resname;
    ...
    RescheduleAdapter clinicavailabilityAdapter;

    public static final String Myschedules = "schMyPrefs" ;
    public static final String SchedulDate = "schnameKey";
    public static final String SchClinicJina = "clinicname";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clinicavailabilityAdapter = new RescheduleAdapter(Reschedule.this, availableclinics);
        recyclerClinicavailability =findViewById(R.id.recyclerClinicavailability);

        if(!(intent.getStringExtra("Resclinicname")==null)){
            resname = intent.getStringExtra("Resclinicname");
            reskey = intent.getStringExtra("Resclinickey");
            clinicname.setText(resname);

        }

        //set shared preferrences
        editor.putString(SchedulDate, daytoday+" "+todayDate);
        editor.putString(SchClinicJina, clinicname.getText().toString());
        editor.commit();
        ...
        recyclerClinicavailability.addOnItemTouchListener(new RecyclerTouchListener(this, recyclerClinicavailability, new CustomItemClickListener() {
            ...
            public void onClick(View view, int position) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    builder = new AlertDialog.Builder(Reschedule.this, android.R.style.Theme_Material_Dialog_Alert);
                } else {
                    builder = new AlertDialog.Builder(Reschedule.this);
                }
                builder.setTitle("Book Clinic")
                        .setMessage("Do want you to Reschedule " + clinname+" on " + todaytxt.getText().toString() + " at " + clickedclinic.getAvailtime() + " ?")
                        .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (todaytxt.getText().toString().equals("Today")) {
                                    ...
                                    chosenclinic = new Chosenclinic(clinicname.getText().toString(), daytoday + " " + todayDate,clickedclinic.getAvailtime(),reskey);
                                } else {
                                    bookedtime = new Bookedtime(clickedclinic.getKeyvalue(), todaytxt.getText().toString());
                                    chosenclinic = new Chosenclinic(clinicname.getText().toString(), todaytxt.getText().toString(),clickedclinic.getAvailtime(),reskey);

                                }
                                //patient specific bookings
                                dbaseref.child("Patients").child(currentFirebaseUser.getUid().toString()).child(reskey).setValue(chosenclinic);
                                //all times booked
                                dbaseref.child("Bookedtimes").child(clinicname.getText().toString()).child(reskey).setValue(bookedtime);
                                finish();
                            }
                        })
...
//                        date.getTime()- will generate the time stamp
                        ...
                        editor.clear();
                        editor.putString(SchedulDate, todaytxt.getText().toString());
                        editor.putString(SchClinicJina, clinicname.getText().toString());
                        editor.commit();
                        fetchData();
